...
.hero-eyebrow{display:inline-flex;align-items:center;gap:0.5rem;border:1px solid rgba(255,255,255,0.15);border-radius:100px;padding:0.3rem 0.875rem;font-size:0.7rem;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;color:rgba(255,255,255,0.5);margin-bottom:1.5rem;}
.hero-eyebrow span{width:6px;height:6px;border-radius:50%;background:#22C55E;display:inline-block;}
.hero h1{font-family:'Lora',serif;font-size:clamp(2rem,4.5vw,3.2rem);font-weight:500;color:var(--white);line-height:1.2;margin-bottom:0.75rem;}
...
.cat-bar{background:var(--white);border-bottom:1px solid var(--line);padding:0 2rem;display:flex;align-items:center;gap:0.5rem;overflow-x:auto;}
...
.fsel{padding:0.35rem 0.875rem;border-radius:8px;border:1.5px solid var(--line);font-family:'Sora',sans-serif;font-size:0.775rem;color:var(--ink);background:var(--white);cursor:pointer;outline:none;}
...
.card{background:var(--white);border-radius:14px;overflow:hidden;border:1px solid var(--line);transition:all 0.2s;cursor:pointer;}
...
.badge{position:absolute;top:0.65rem;left:0.65rem;padding:0.2rem 0.6rem;border-radius:5px;font-size:0.65rem;font-weight:700;letter-spacing:0.8px;text-transform:uppercase;}
...
.badge-feat{position:absolute;top:0.65rem;right:0.65rem;background:rgba(255,255,255,0.95);color:var(--amber);padding:0.2rem 0.5rem;border-radius:5px;font-size:0.65rem;font-weight:700;}
...
.card-title{font-size:0.875rem;font-weight:600;color:var(--ink);margin-bottom:0.3rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.card-meta{font-size:0.75rem;color:var(--ink3);display:flex;align-items:center;gap:0.35rem;}
.card-footer{padding:0.75rem 1.1rem;border-top:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;}
.card-owner{font-size:0.75rem;color:var(--ink3);}
.card-date{font-size:0.7rem;color:var(--ink3);}
.overlay{position:fixed;inset:0;background:rgba(26,26,46,0.65);z-index:200;display:flex;align-items:center;justify-content:center;padding:1rem;backdrop-filter:blur(4px);}
.modal{background:var(--white);border-radius:18px;max-width:600px;width:100%;max-height:92vh;overflow-y:auto;position:relative;}
...
.sidebar-section{font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:rgba(255,255,255,0.3);margin:1.5rem 0 0.5rem;padding-left:0.5rem;}
.slink{display:flex;align-items:center;gap:0.65rem;padding:0.6rem 0.75rem;border-radius:7px;font-size:0.825rem;font-weight:500;color:rgba(255,255,255,0.55);cursor:pointer;transition:all 0.12s;border:none;background:transparent;width:100%;text-align:left;}
...
.atbl th{padding:0.75rem 1rem;text-align:left;font-size:0.7rem;text-transform:uppercase;letter-spacing:1px;color:var(--ink3);border-bottom:1px solid var(--line);background:var(--bg);}
  ...
  return (
    <div className="card" onClick={() => onClick(l)}>
      ...
      <div className="card-body">
        <div className="card-price"><sup>$</sup>{n}<sub> {l.currency}{suffix}</sub></div>
        <div className="card-title">{l.title}</div>
        <div className="card-meta">📍 {l.canton}, {l.province}</div>
      </div>
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-img"><img src={l.img} alt={l.title} /></div>
...
const HomeView = ({ onNav }) => {
  ...
  return (
    <>
      <div className="hero">
        <div className="hero-eyebrow"><span />Plataforma de clasificados · Costa Rica</div>
        <h1>Anuncia lo que tienes.<br /><em>Encuentra lo que buscas.</em></h1>
        ...
        <div className="hero-stats">
          <div><div className="hstat-n">1,200+</div><div className="hstat-l">Anuncios</div></div>
          <div><div className="hstat-n">7</div><div className="hstat-l">Provincias</div></div>
        ...
        <div className="cat-cards">
          {Object.entries(FORM_CATS).map(([id, c]) => (
            <div key={id} className={`cat-card ${cat === id ? 'active' : ''}`} onClick={() => { setCat(id); set('sub', ''); set('op', ''); }}>
              <div className="cat-card-icon">{id === 'bienes-raices' ? '⌂' : id === 'vehiculos' ? '◉' : '◆'}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
              <div className="fgrid">
                <div className="fgroup">
                  ...
                  <select className="fselect" value={form.sub} onChange={(e) => set('sub', e.target.value)}>
                    <option value="">Seleccionar...</option>
                    {cfg.subs.map((s) => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div className="fgroup">
                  <label className="flabel">Operación</label>
                  <select className="fselect" value={form.op} onChange={(e) => set('op', e.target.value)}>
                    <option value="">Seleccionar...</option>
                    {cfg.ops.map((o) => <option key={o}>{o}</option>)}
                  </select>
                </div>
              </div>
              <div className="fgroup">
          ...
          <div className="fsec">
            ...
            <div className="fgrid">
              <div className="fgroup">
                ...
                <input className="finput" placeholder="+506 8888-0000" value={form.phone} onChange={(e) => set('phone', e.target.value)} />
              </div>
              <div className="fgroup">
                <label className="flabel">Correo electrónico</label>
                <input className="finput" type="email" placeholder="tu@correo.com" value={form.email} onChange={(e) => set('email', e.target.value)} />
              </div>
            </div>
          </div>
          {error && <div className="auth-error">{error}</div>}
  ...
  return (
    <div className="admin-wrap">
      <div className="sidebar">
        <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', paddingLeft: '0.5rem' }}>Anunciacr</div>
        <div className="sidebar-section">Plataforma</div>
        {[['📊', 'Dashboard'], ['📋', 'Anuncios'], ['👥', 'Usuarios']].map(([ic, lb]) => (
          <button key={lb} className={`slink ${lb === 'Dashboard' ? 'active' : ''}`}>{ic} {lb}</button>
        ))}
      </div>
      <div className="admin-main">
        ...
        <div className="kpis">
          ...
          <div className="kpi"><div className="kpi-l">Vehículos</div><div className="kpi-v">{veh}</div><div className="kpi-d">{Math.round((veh / total) * 100)}% del total</div></div>
          <div className="kpi"><div className="kpi-l">Empleos</div><div className="kpi-v">{emp}</div><div className="kpi-d">{Math.round((emp / total) * 100)}% del total</div></div>
        </div>
        <div style={{ fontWeight: 600, marginBottom: '0.875rem', fontSize: '0.9rem' }}>Anuncios recientes</div>
        <div className="table-wrap">
          <table className="atbl">
            <thead><tr><th>Anuncio</th><th>Categoría</th><th>Precio</th><th>Provincia</th><th>Estado</th><th></th></tr></thead>
            <tbody>
              {LISTINGS.map((l) => (
                <tr key={l.id}>
                  <td><div style={{ fontWeight: 500 }}>{l.title}</div><div style={{ fontSize: '0.75rem', color: 'var(--ink3)' }}>{l.owner}</div></td>
                  <td><span className={`cat-pill ${pillClass(l.cat)}`}>{catLabel(l.cat)}</span></td>
                  <td style={{ fontWeight: 600 }}>${l.price.toLocaleString()}</td>
                  <td style={{ color: 'var(--ink2)' }}>{l.province}</td>
                  <td><span className="status"><span className={`dot ${l.featured ? 'dot-active' : 'dot-paused'}`} />{l.featured ? 'Destacado' : 'Activo'}</span></td>
                  <td><button className="act-btn">✕</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function App() {
  ...
  return (
    <>
      <style>{S}</style>
      <nav className="nav">
        <div className="logo" onClick={() => setView('home')}>Anuncia<span>cr</span><em>.com</em></div>
        <div className="nav-links">
          <button className={`nav-link ${view === 'home' ? 'active' : ''}`} onClick={() => setView('home')}>Explorar</button>
          {user ? (
            <>
